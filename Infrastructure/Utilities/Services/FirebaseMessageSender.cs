...
namespace Utilities
{
    public class FirebaseMessageSender : IFirebaseMessageSender
    {
        ...
        public FirebaseMessageSender(IOptions<FirebaseSetting> firebaseOptions)
        {
            try
            {
                var privateKeyValue = firebaseOptions.Value.private_key.Replace("\\n", "\n");
                firebaseOptions.Value.private_key = privateKeyValue;

                var settings = JsonSerializer.Serialize(firebaseOptions.Value);
