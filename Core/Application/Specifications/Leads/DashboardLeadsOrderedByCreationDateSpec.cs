...
namespace Application.Specifications.Leads
{
    ...
    {
        public DashboardLeadsOrderedByCreationDateSpec(string lang, string name, LeadStatuses? status)
        {
            Query.Where(lead => (string.IsNullOrWhiteSpace(name) || lead.LeadName.Contains(name)) &&
